...
I gained valuable experience during my two semesters working on both reactive and pre-mapping based controls for this competition. This practical experience allowed me to apply and reshape the theoretical concepts I learned in my classes at UPenn. My expertise in creating software pipelines and using ROS (Robot Operating System) also proved beneficial.

Our objective was to navigate the track without relying on a pre-mapped path for the reactive-based controls aspect of the competition. Instead, we aimed to react to the observed track in real-time, ensuring safe operation for five laps to secure victory. Initially, we utilized LIDAR sensors to gather information about the road and cone locations using RANSAC and clustering techniques respectively. However, due to the noise and inconsistency in the LIDAR detections, we faced challenges in trusting this approach. To overcome this, we decided to leverage a single monocular camera. By converting its image into a pre-calibrated bird's eye view and adjusting the depth measurements on a per-pixel basis, we obtained more reliable information. The Canny edge detection algorithm was then used to identify grass regions, which were further converted into a Laser Scan format. This consistent Laser Scan was then fed into the gap-follow algorithm. Our approach performed well, particularly under favorable lighting conditions. You can watch the back-end visualization of our algorithm and the actual run below.


<iframe width="140" height="75" src="https://www.youtube.com/embed/xCRqczp_Acs" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
<iframe width="140" height="75" src="https://www.youtube.com/embed/6IgGBG_flno" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

In the context of pre-mapping based controls, my responsibility was to develop a lightweight Python library that could fuse multiple sensors, including GPS, IMU, and Wheel Odometry, using a partial state update. You might wonder why we chose to develop a separate library instead of utilizing existing open-source options. This is a valid question, and initially, we started the project by using the well-known and open-source Robot Localization library in ROS. However, after considering its abstract nature, we concluded that using this library would be excessive for our requirements. Inspired by its approach, I developed a partial update scheme for the Extended Kalman Filter (EKF) algorithm that served our purpose effectively.

Integrating this localization approach with the pure-pursuit algorithm involved creating pre-mapping pipelines. Initially, we manually drove the go-kart, collecting waypoints along the track using our sensor fusion. These waypoints were then utilized to create a path for the pure-pursuit algorithm, employing a simple linear interpolation scheme. This allowed the pure-pursuit algorithm to follow the desired trajectory accurately. Please watch our pure-pursuit winning run in the competition below.

<iframe width="140" height="75" src="https://www.youtube.com/embed/CzRc5b1CP3M" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
